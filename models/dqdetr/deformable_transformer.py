...
#         else:
#             # 调试打印，如果是训练阶段但 real_counts 为 None
#             if self.training and random.random() < 0.01:
#                 print(f"[Transformer DEBUG] Training={self.training}, dn_targets exists={dn_targets is not None}")
#
#         # 3. 执行动态查询模块
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, dn_targets, args_dn):
        ...
        # 5. 准备 DN (Denoising)
        # 这一步必须在两阶段逻辑之前执行，以便 dn_meta 准备就绪
        tgt, refpoint_embed, attn_mask, dn_meta = prepare_for_cdn(
            dn_args=(dn_targets, args_dn[0], args_dn[1], args_dn[2]),
            training=args_dn[3], num_queries=num_select, num_classes=args_dn[4],
            hidden_dim=args_dn[5], label_enc=args_dn[6]
        ...
        # [关键修复] 6. 注入 DQ 输出到 dn_meta
        if dn_meta is None: dn_meta = {}
        dn_meta['dq_outputs'] = dq_outputs
        dn_meta['real_counts'] = real_counts

        # --------------------------------------------------------------------------------
        # 处理两阶段逻辑 (Two-Stage Logic)
        # 注意：此处我们需要覆盖原有的 refpoint_embed 和 tgt，使用动态生成的
        # --------------------------------------------------------------------------------
        if self.two_stage_type == 'standard':
            # A. 动态查询替换逻辑
            # 将动态生成的参考点 (0-1) 转换为 unsigmoid 格式
            refpoint_embed_unsigmoid = inverse_sigmoid(refpoint_embed_dynamic)

            # 使用动态参考点作为初始 Proposal
            init_box_proposal = refpoint_embed_dynamic  # sigmoid 格式

            # B. 生成/获取对应的 Content Query (Target)
            # 方案：使用 Learnable Embedding (如果启用) 或 零初始化
            if self.embed_init_tgt:
                bs = memory.shape[0]
                # 动态截取前 num_select 个 learnable query
                tgt_dynamic = self.tgt_embed.weight[:num_select].unsqueeze(1).repeat(1, bs, 1).transpose(0, 1)
            else:
                # 如果没有 learnable tgt，则初始化为 0 (或者从 memory 中 gather，这里简化为 0)
                bs = memory.shape[0]
                tgt_dynamic = torch.zeros(bs, num_select, self.d_model, device=memory.device)

            # C. 结合 DN 部分
            if refpoint_embed is not None:
                # refpoint_embed 是 DN 的部分 (unsigmoid)
                # refpoint_embed_unsigmoid 是动态查询的部分 (unsigmoid)
                refpoint_embed = torch.cat([refpoint_embed, refpoint_embed_unsigmoid], dim=1)
                tgt = torch.cat([tgt, tgt_dynamic], dim=1)
            else:
                refpoint_embed = refpoint_embed_unsigmoid
                tgt = tgt_dynamic

            # 注意：原代码中的 gen_encoder_output_proposals 和 enc_out_class_embed 逻辑
            # 在这里被动态查询模块替代了。原逻辑是基于全图密集预测选取 TopK，
            # 现在 DynamicQueryModule 已经帮我们做好了这件事。

            # 保留必要的 encoder outputs 用于 auxiliary loss (如果需要)
            # 为了兼容性，我们可以构建伪造的 hs_enc
            # 但这里最重要的是传递正确的 tgt 和 refpoint 给 decoder

            # 为了支持 encoder loss (如果 config 中开启)，我们需要保留原逻辑计算 output_memory
            # 但为了避免干扰主逻辑，我们只计算它用于返回，不用于 Decoder 输入
            if self.two_stage_keep_all_tokens:
                # 原逻辑备用 (省略，通常为 False)
                hs_enc = None
                ref_enc = None
            else:
                # 简单返回 tgt 作为 hs_enc 占位
                hs_enc = tgt_dynamic.unsqueeze(0)
                ref_enc = init_box_proposal.unsqueeze(0)  # 这是一个近似
            ...
            if self.num_patterns > 0:
                # Pattern 逻辑 (略，假设 num_patterns=0)
                pass
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        dynamic_query_levels=getattr(args, 'dynamic_query_levels', [300, 500, 900, 1500]),
