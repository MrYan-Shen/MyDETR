...
#
#     # [修复] 显式添加动态查询损失的权重
#     # 即使 args 中没有定义，也给予默认值，确保梯度回传
#     if hasattr(args, 'dq_ce_loss_coef'):
#         weight_dict['loss_dq_ce'] = args.dq_ce_loss_coef
#     else:
#         weight_dict['loss_dq_ce'] = 1.0
#         print("Warning: args.dq_ce_loss_coef not found, using default 1.0")
#
#     if hasattr(args, 'dq_l2_loss_coef'):
#         weight_dict['loss_dq_l2'] = args.dq_l2_loss_coef
#     else:
#         weight_dict['loss_dq_l2'] = 0.5
#         print("Warning: args.dq_l2_loss_coef not found, using default 0.5")
#
...
from util.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       accuracy, get_world_size, interpolate,
                       is_dist_avail_and_initialized, inverse_sigmoid, is_main_process)  # Added is_main_process
from .backbone import build_backbone
from .matcher import build_matcher
from .segmentation import (DETRsegm, PostProcessPanoptic, PostProcessSegm,
                           dice_loss)
...
class SetCriterion(nn.Module):
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, dq_query_levels=[300, 500, 900, 1500]):
        super().__init__()
        self.num_classes = num_classes
        self.matcher = matcher
        self.weight_dict = weight_dict
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if self.training and dn_meta is not None and 'dq_outputs' in dn_meta:
            ...
            # else:
            #     if is_main_process():
            #         print(f"[Diagnostic] Skip CE Loss: probs={interval_probs is not None}, counts={real_counts is not None}")
...
def build_dqdetr(args):
    ...
    dq_query_levels = getattr(args, 'dynamic_query_levels', [300, 500, 900, 1500])

    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict,
                             focal_alpha=args.focal_alpha, losses=losses,
                             dq_query_levels=dq_query_levels)
    criterion.to(device)
    postprocessors = {'bbox': PostProcess(nms_iou_threshold=args.nms_iou_threshold)}
    if args.masks:
        postprocessors['segm'] = PostProcessSegm()
